...
gxf::Expected<void> ConvertTensorToVideoBuffer(NetworkOutputType network_output_type,
                                               ...
                                               cudaStream_t stream) {
  gxf::Expected<const T*> in_tensor_data = in_tensor->data<T>();
  if (!in_tensor_data) {
  ...
  CopyTensorData<T>(network_output_type, data_format, shape, in_tensor_data.value(),
...
gxf_result_t SegmentationPostprocessor::tick() {
  ...
  if (network_output_type_value_ == NetworkOutputType::kArgmax) {
    switch (in_tensor->element_type()) {
      case gxf::PrimitiveType::kInt32:
        maybe_kernel_result =
            ConvertTensorToVideoBuffer<int32_t>(network_output_type_value_, data_format_value_, shape, in_tensor,
                                      out_video_buffer.value(), cuda_stream);
        break;
      case gxf::PrimitiveType::kInt64:
        maybe_kernel_result =
            ConvertTensorToVideoBuffer<int64_t>(network_output_type_value_, data_format_value_, shape, in_tensor,
                                      out_video_buffer.value(), cuda_stream);
        break;
      default:
        GXF_LOG_ERROR("Unsupported tensor element type.");
        return GXF_FAILURE;
    }
  } else {
  maybe_kernel_result =
