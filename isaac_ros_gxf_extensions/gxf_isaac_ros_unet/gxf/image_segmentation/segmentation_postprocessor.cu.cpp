...
template <DataFormat data_format, typename T>
__global__ void CopyTensorDataCuda(Shape shape, const T* input, output_type_t* output,
                                   NetworkOutputType network_output_type) {
  const uint32_t x = blockIdx.x * blockDim.x + threadIdx.x;
  const uint32_t y = blockIdx.y * blockDim.y + threadIdx.y;
...
void CopyTensorData(NetworkOutputType network_output_type, DataFormat data_format, Shape shape,
  ...
  switch (data_format) {
    case DataFormat::kNCHW:
      CopyTensorDataCuda<DataFormat::kNCHW, T>
          <<<grid, block, 0, stream>>>(shape, input, output, network_output_type);
      break;
    case DataFormat::kHWC:
      CopyTensorDataCuda<DataFormat::kHWC, T>
          <<<grid, block, 0, stream>>>(shape, input, output, network_output_type);
      break;
    case DataFormat::kNHWC:
      CopyTensorDataCuda<DataFormat::kNHWC, T>
          <<<grid, block, 0, stream>>>(shape, input, output, network_output_type);
      break;
  }
}
template void CopyTensorData<int32_t>(NetworkOutputType network_output_type, DataFormat data_format, Shape shape,
                    const int32_t* input, output_type_t* output, cudaStream_t stream);
template void CopyTensorData<int64_t>(NetworkOutputType network_output_type, DataFormat data_format, Shape shape,
                    const int64_t* input, output_type_t* output, cudaStream_t stream);
